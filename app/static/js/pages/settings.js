// Extracted from settings.html for browser caching.
// window.SETTINGS_URLS is set by a small inline script before this loads.

function serverPartialUrl(panel){ return window.SETTINGS_URLS.serverPartialBase.replace('__PANEL__', panel); }
function dataPartialUrl(panel){ return window.SETTINGS_URLS.dataPartialBase.replace('__PANEL__', panel); }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function unifiedSaveServerConfig(formEl){
        ...
        try {
            const endpoint = serverPartialUrl('config');
    ...
    function ensureServerAIBindings(){
        ...
        if(typeof window.__initAISettings === 'function'){
            try { window.__initAISettings(); } catch (err){ console.debug('AI init helper failed', err); }
        }
    }
    ...
    function ensureServerSMTPBindings(){
        ...
        if(testBtn && !testBtn.dataset.bound){
            testBtn.addEventListener('click', async (ev) => {
                ...
                try {
                    const resp = await fetch(window.SETTINGS_URLS.testSmtpConnection, {
                        method: 'POST',
                        body: new FormData(form),
                        credentials: 'same-origin'
                    });
                ...
                setTimeout(()=>{
                    try {
                        if(typeof loadServerPanel === 'function'){
                            ...
                            const endpoint = root.dataset.refreshUrl || serverPartialUrl('opds');
    ...
    fetch(`/admin/api/users/${uid}/delete`, {method:'POST', body:fd, credentials:'same-origin'})
        ...
        function loadServerPanel(panel){
                if(!serverContainer) return;
                let endpoint;
                if(panel==='config') endpoint = serverPartialUrl('config');
                if(panel==='users') endpoint = serverPartialUrl('users');
                if(panel==='debug') endpoint = serverPartialUrl('debug');
                if(panel==='ai') endpoint = serverPartialUrl('ai');
                if(panel==='smtp') endpoint = serverPartialUrl('smtp');
                if(panel==='backup') endpoint = serverPartialUrl('backup');
                if(panel==='opds') endpoint = serverPartialUrl('opds');
                if(panel==='audiobookshelf') endpoint = serverPartialUrl('audiobookshelf');
                if(panel==='metadata') endpoint = serverPartialUrl('metadata');
                if(panel==='repairs') endpoint = serverPartialUrl('repairs');
                if(panel==='jobs') endpoint = serverPartialUrl('jobs');
                                                                        ...
                                                                        window.__initAudiobookshelfPanel = function(){
                                                                                ...
                                                                                if (testBtn) {
                                                                                        testBtn.addEventListener('click', function(){
                                                                                            ...
                                                                                            fetch(window.SETTINGS_URLS.testAudiobookshelfConnection, { method:'POST', credentials:'same-origin', headers })
                                                                                                .then(r=>r.json()).then(j=>{
                                                                                                    if(status) status.textContent=(j.ok?'OK: ':'Fail: ')+(j.message||'');
                                                                                        fetch((serverPartialUrl('audiobookshelf') + '?test=1'), { credentials:'same-origin' })
                                                                                            .then(r=>r.text()).then(html=>{ serverContainer.innerHTML = html; if (window.__initAudiobookshelfPanel) window.__initAudiobookshelfPanel(); });
                                                                                ...
                                                                                if (syncTestBtn) {
                                                                                        syncTestBtn.addEventListener('click', async function(){
                                                                                            ...
                                                                                            try {
                                                                                                const resp = await fetch(window.SETTINGS_URLS.audiobookshelfTestSync, {method:'POST', headers, credentials:'same-origin', body: JSON.stringify({limit:5})});
                                                                                ...
                                                                                if (listenTestBtn) {
                                                                                        listenTestBtn.addEventListener('click', async function(){
                                                                                            ...
                                                                                            const headers={'Content-Type':'application/json'}; const tok=getCsrf(); if(tok) headers['X-CSRFToken']=tok;
                                                                                            try {
                                                                                                const resp = await fetch(window.SETTINGS_URLS.audiobookshelfListenTest, {method:'POST', headers, credentials:'same-origin', body: JSON.stringify({page_size: 200})});
                                                                                                const j = await resp.json();
                                                                                                if(j.ok){ try{ startInlineProgressPolling(j); }catch(e){}
                                                                                                    const to = j.progress_url || j.api_progress_url || '#'; if(to && to !== '#'){ setTimeout(()=>{ window.location.assign(to); }, 150); }
                                                                                ...
                                                                                if (fullSyncBtn) {
                                                                                        fullSyncBtn.addEventListener('click', async function(){
                                                                                            ...
                                                                                            const headers={'Content-Type':'application/json'}; const tok=getCsrf(); if(tok) headers['X-CSRFToken']=tok;
                                                                                            try {
                                                                                                const resp = await fetch(window.SETTINGS_URLS.audiobookshelfFullSync, {method:'POST', headers, credentials:'same-origin'});
                                                                                                const j = await resp.json();
                                ...
                                window.__initABSPanel = function(){
                                        ...
                                        testBtn.addEventListener('click', function(){
                                            ...
                                            fetch('/settings/audiobookshelf/test', { method:'POST', headers, credentials:'same-origin' })
                                              ...
                                              .then(j=>{
                                                  ...
                                                  fetch((serverPartialUrl('audiobookshelf') + '?test=1'))
                                                    .then(r=>r.text())
                                                    .then(html=>{ container.innerHTML = html; window.__initABSPanel(); });
                                              })
                                              .catch(()=>{ if(status) status.textContent = 'Test failed'; });
                                    ...
                                    if(syncTestBtn && !syncTestBtn.dataset.bound){
                                        syncTestBtn.addEventListener('click', function(){
                                            ...
                                            fetch(window.SETTINGS_URLS.audiobookshelfTestSync, { method:'POST', headers, credentials:'same-origin', body: JSON.stringify({limit:5}) })
                                              .then(r=>r.json().catch(()=>({ok:false,message:'bad json'})))
                                              .then(j=>{
                                    ...
                                    if(listenTestBtn && !listenTestBtn.dataset.bound){
                                        listenTestBtn.addEventListener('click', function(){
                                            if(status) status.textContent = 'Starting listening-only test...';
                                            const headers = csrf ? {'X-CSRFToken': csrf, 'Content-Type':'application/json'} : {'Content-Type':'application/json'};
                                            fetch(window.SETTINGS_URLS.audiobookshelfListenTest, { method:'POST', headers, credentials:'same-origin', body: JSON.stringify({page_size:200}) })
                                              .then(r=>r.json().catch(()=>({ok:false,message:'bad json'})))
                                              .then(j=>{
                        ...
                        if(panel==='users'){
                            ...
                            if(typeof window.editUserInline !== 'function'){
                                window.editUserInline = function(uid){
                                    const container = document.getElementById('serverDynamicContainer');
                                    if(!container) return;
                                    const url = (serverPartialUrl('user_edit') + '?user_id=')+encodeURIComponent(uid);
                            ...
                            if(typeof window.loadServerUsersFull !== 'function'){
                                window.loadServerUsersFull = function(search){
                                    ...
                                    const urlObj = new URL(serverPartialUrl('users'), window.location.origin);
                        ...
                        if(panel==='user_edit'){
                            if(typeof window.submitUserEdit !== 'function'){
                                window.submitUserEdit = function(form){
                                    const fd = new FormData(form);
                                    fetch(serverPartialUrl('user_edit'), {method:'POST', body:fd, credentials:'same-origin'})
                                      .then(r=>r.text())
    ...
    function loadPersonal(panel){
        if(!personalContainer) return;
            const routes = {
                'profile': window.SETTINGS_URLS.profilePartial,
                'password': window.SETTINGS_URLS.passwordPartial,
                'privacy': window.SETTINGS_URLS.privacyPartial,
                'reading_prefs': window.SETTINGS_URLS.readingPrefsPartial,
                'personal_abs': window.SETTINGS_URLS.personalAbsPartial,
                'activity': window.SETTINGS_URLS.myActivity // fallback link
            };
            if(panel==='activity'){
